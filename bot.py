...
# GROUP_CHAT_ID: bot faqat shu guruhda ishlashi kerak bo'lsa, guruhning chat ID sini
# shu yerga kiriting (masalan: -1001234567890). Bo'sh qoldirilsa - bot qaysi chatdan
# yozilishidan qatiy nazar ishlaydi (guruh + shaxsiy xabarlar).
...
# Bo'sh qoldirilsa - guruhdagi barcha xodimga ruxsat beriladi.
ADMIN_IDS = set(int(x) for x in os.environ.get('ADMIN_IDS', '').split(',') if x.strip())

def has_permission(update: Update):
    chat_ok = not GROUP_CHAT_ID or update.effective_chat.id == GROUP_CHAT_ID
    user_ok = not ADMIN_IDS or update.effective_user.id in ADMIN_IDS
